...
idaman void ida_export eseek(FILE *fp,int32 pos);
...
idaman void ida_export echsize(FILE *fp,uint32 size);


// Get free disk space in bytes
//      path - name of any directory on the disk to get information about

idaman uint64 ida_export getdspace(const char *path);
...
idaman bool ida_export choose_ioport_device(
        const char *file,
        char *device,
        size_t device_size,
...
uint32 get_thread_priority(void);        // 0 - something wrong (can't happen
                                        // under OS/2)

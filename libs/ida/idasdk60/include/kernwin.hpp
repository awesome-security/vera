...
struct exec_request_t
{
  ...
  bool valid(void) const { return (code & ~7) == MFF_MAGIC && sem != NULL; }
...
inline void setup_range_marker(void)
{
  ...
  if ( ptr != NULL )
    range_marker = (void (idaapi*)(ea_t, asize_t))ptr;
}
...
AS_PRINTF(5, 0) inline int askbuttons_cv(
        const char *Yes,
        const char *No,
        const char *Cancel,
        int deflt,
...
AS_PRINTF(1, 2) inline void ask_for_feedback(const char *format,...)
{
  va_list va;
  va_start(va, format);
...
AS_PRINTF(1, 0) inline void vwarning(const char *format, va_list va)
{
  callui(ui_mbox, mbox_warning, format, va);
}

AS_PRINTF(1, 0) inline void vinfo(const char *format, va_list va)
{
  callui(ui_mbox, mbox_info, format, va);
}

AS_PRINTF(1, 0) NORETURN inline void vnomem(const char *format, va_list va)
{
...
AS_PRINTF(1, 0) inline int vmsg(const char *format, va_list va)
{
...
AS_PRINTF(1, 2) NORETURN inline void error(const char *format,...)
{
  va_list va;
  va_start(va, format);
  verror(format, va);
...
AS_PRINTF(1, 2) inline void warning(const char *format, ...)
{
  va_list va;
  va_start(va, format);
  vwarning(format, va);
...
AS_PRINTF(1, 2) inline void info(const char *format, ...)
{
  va_list va;
  va_start(va, format);
  vinfo(format, va);
...
AS_PRINTF(1, 2) NORETURN inline void nomem(const char *format, ...)
{
  va_list va;
  va_start(va, format);
  vnomem(format, va);
...
AS_PRINTF(1, 2) inline int msg(const char *format, ...)
{
  va_list va;
  va_start(va, format);
...
idaman void ida_export qstr2user(qstring *dst, const qstring &src);
idaman void ida_export user2qstr(qstring *dst, const qstring &src);
...
inline void *idaapi unpack_obj(const uchar **pptr, const uchar *end, void *destbuf, size_t destsize)
{
...
inline void *idaapi unpack_buf(const uchar **pptr, const uchar *end, size_t *sz)
{
...
inline const void *idaapi unpack_obj_inplace(const uchar **pptr, const uchar *end, size_t objsize)
{
...
inline const void *idaapi unpack_buf_inplace(const uchar **pptr, const uchar *end, size_t *sz)
{
...
inline void unpack_eavec(ea_t ea, eavec_t &insns, const uchar **ptr, const uchar *end)
{
  ea_t old = ea;
...
inline uint64 unpack_uleb128(const uchar **pptr, const uchar* end)
{
  uint64 result = 0;
  int  bit = 0;
  const uchar *ptr = *pptr;
...
inline int64 unpack_sleb128(const uchar **pptr, const uchar *end)
{
  int64 result = 0;
  int bit = 0;
  const uchar *ptr = *pptr;
...
inline void append_obj(bytevec_t &v, const void *obj, size_t size)
{
  v.append(obj, size);
}
...
inline void append_ds(bytevec_t &v, const char *x)
{
  size_t len = strlen(x);
#ifdef __X64__
  QASSERT(len <= 0xFFFFFFFF);
#endif
  append_dd(v, uint32(len));
  append_obj(v, x, len);
}

inline void append_buf(bytevec_t &v, const void *buf, size_t len)
{
#ifdef __X64__
  QASSERT(len <= 0xFFFFFFFF);
#endif
...
inline void append_eavec(bytevec_t &v, ea_t ea, const eavec_t &insns)
{

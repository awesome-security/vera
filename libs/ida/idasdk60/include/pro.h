...
#define IDA_SDK_VERSION      600        // IDA SDK v6.0
...
idaman const wchar16_t *ida_export qstrchr(const wchar16_t *s1, wchar16_t c);
...
idaman const wchar16_t *ida_export qstrrchr(const wchar16_t *s1, wchar16_t c);
...
idaman size_t ida_export parse_command_line(const char *cmdline, qstrvec_t *args);
...
idaman bool ida_export win_utf2idb(char *buf);
...
idaman void *ida_export init_process(const launch_process_t &lpi, qstring *errbuf);


// Forcibly terminate a running process
// Returns: 0-ok, otherwise an error code that can be passed to winerr()

idaman int ida_export term_process(void *handle);
...
idaman int ida_export get_process_exit_code(void *handle, int *exit_code);

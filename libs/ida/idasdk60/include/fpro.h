...
idaman int32 ida_export qftell(FILE *fp);
idaman int   ida_export qfseek(FILE *fp, int32 offset, int whence);                  /* 0-Ok */
...
idaman int   ida_export qrename(const char *oldfname, const char *newfname); // newname may exist, will be deleted
...
idaman AS_PRINTF(2, 0) int ida_export qvfprintf(FILE *fp, const char *format, va_list va); // out to FILE*
idaman AS_PRINTF(1, 0) int ida_export qvprintf(const char *format, va_list va);    // out to stdout
idaman AS_PRINTF(1, 0) int ida_export qveprintf(const char *format, va_list va);   // out to stderr
idaman AS_SCANF (2, 0) int ida_export qvfscanf(FILE *fp, const char *format, va_list va);
idaman char *ida_export qgets(char *line, size_t linesize);
int idaapi qgetchar(void);
#ifdef __cplusplus
AS_PRINTF(2, 3) inline int qfprintf(FILE *fp, const char *format, ...)
{
  va_list va;
  va_start(va, format);
...
AS_PRINTF(1, 2) inline int qprintf(const char *format, ...)
{
  va_list va;
  va_start(va, format);
...
AS_PRINTF(1, 2) inline int qeprintf(const char *format, ...)
{
  va_list va;
  va_start(va, format);
...
AS_SCANF(2, 3) inline int qfscanf(FILE *fp, const char *format, ...)
{
  va_list va;
  va_start(va, format);

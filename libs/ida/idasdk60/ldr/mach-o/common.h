...
class macho_file_t
{
  ...
  bool processExportNode(const uchar *start, const uchar *p, const uchar *end, char *symname, int symnameoff, dyld_info_visitor_t &v);
  bool visit_export_info(const bytevec_t &data, dyld_info_visitor_t &v);

public:
  macho_file_t(linput_t *_li, size_t _start_offset = 0):
      li(_li), mach_offset(-1), mach_size(0),
      start_offset(_start_offset), parsed_section_info(false) {};

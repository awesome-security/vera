...
static bool m64; //current file is 64-bit
//--------------------------------------------------------------------------
...
/*
#ifndef EFD_COMPILE
local void swap_arm_exception_state(arm_exception_state_t *exc)
{
        exc->__exception = SWAP_LONG(exc->__exception);
        exc->__fsr = SWAP_LONG(exc->__fsr);
        exc->__far = SWAP_LONG(exc->__far);
}

//--------------------------------------------------------------------------
local void swap_arm_vfp_state(arm_vfp_state_t *fpu)
{
        for(int i = 0; i < 32; i++)
            fpu->__r[i] = SWAP_LONG(fpu->__r[i]);
        fpu->__fpscr = SWAP_LONG(fpu->__fpscr);
}
#endif // EFD_COMPILE
*/
...
bool macho_file_t::parse_load_commands()
{
  ...
  if ( commands_end != begin )
    msg("Inconsistent mh_sizeofcmds\n");
...
//--------------------------------------------------------------------------
bool macho_file_t::processExportNode(const uchar *start, const uchar *p, const uchar *end, char *symname, int symnameoff, dyld_info_visitor_t &v)
{
  const uchar terminalSize = *p++;
  ...
  const uchar childrenCount = *children++;
  ...
  for (int i=0; i < childrenCount; ++i)
  {
    ...
    while ( *s != '\0' )
    {
      symname[symnameoff+edgeStrLen] = *s++;
      ++edgeStrLen;
    }
    symname[symnameoff+edgeStrLen] = *s++;
    uint32_t childNodeOffset = (uint32_t)unpack_uleb128(&s, end);
    processExportNode(start, start+childNodeOffset, end, symname, symnameoff+edgeStrLen, v);
  }
  return true;
}
...
bool macho_file_t::visit_export_info(const bytevec_t &data, dyld_info_visitor_t &v)
{
  ...
  const uchar *begin = &data[0];
  const uchar *end = begin + data.size();
  processExportNode(begin, begin, end, symname, 0, v);
  return true;
}

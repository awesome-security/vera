...
//
  ...
  uint32 *delta = qnewarray(uint32, size_t(ahd->num_syms));
  if ( delta == NULL )
    nomem();
  ...
  for ( i=0; i < ahd->num_areas; i++,ah++ )
  {
    ...
    for ( int j=0; j < ah->num_relocs; j++ )
    {
      ...
      else
      {
        ...
        target = rvalue;
        if ( rvalue == BADADDR )
          loader_failure("Bad reference to area %d at file offset %x",sid, int(qltell(li)-sizeof(reloc_t)));
        fd.type = 0;
      }
      ...
      switch ( r.type & RF_FT )
      {
        ...
        case RF_FT_INSN: // 11 the field to be relocated is an instruction or instruction sequence
          reloc_insn(ea,(uint32)rvalue,r.type);
          break;
      }
      set_fixup(ea,&fd);
    }

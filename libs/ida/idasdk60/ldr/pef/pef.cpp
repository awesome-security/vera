...
static void process_loader(char *ptr, pef_section_t *sec, int nsec) {
  ...
  for ( i=0; i < pl.relocSectionCount; i++ )
  {
    uint32 sea = sec[prh[i].sectionIndex].defaultAddress;
    uint16 *ptr = relptr + prh[i].firstRelocOffset;
    uint32 reladdr = sea;
    uint32 import  = 0;
    uint32 code    = nsec > 0 ? sec[0].defaultAddress : 0;
    uint32 data    = nsec > 1 ? sec[1].defaultAddress : 0;
    int32 repeat   = -1;
    for ( int j=0; j < prh[i].relocCount; ) {
      ...
      switch ( insn >> 9 ) {
        ...
        case kPEFRelocLgRepeat+1:
          {
            ushort next = mfshort(ptr[j++]);
            if ( repeat == -1 ) repeat = next | (uint32(insn & 0x3F) << 16);
            repeat--;
...
//--------------------------------------------------------------------------
static uint32 read_number(const char *&packed)
{
  uint32 arg = 0;
  for ( int i=0; ; i++ ) {
    uchar b = *packed++;
        ...
        uint32 usize)
{
  ...
  if ( unpacked == NULL )
    nomem("section usize %u\n", usize);
  uint32 saved_usize = usize;
  char *ptr = unpacked;
...
//        msg("%02X (Copy) Arg=%lX usize=%lX\n",code, arg, usize);
      ...
      case 2:           // repeatedBlock
        {
          uint32 repeat = read_number(packed) + 1;
//          msg("%02X (repeatBlock) Arg=%lX Repeat=%lX usize=%lX\n",code, arg, repeat, usize);
      ...
      case 3:           // interleaveRepeatBlockWithBlockCopy
        {
          uint32 commonSize  = arg;
          uint32 customSize  = read_number(packed);
          uint32 repeatCount = read_number(packed);
...
void load_file(linput_t *li, ushort /*neflag*/, const char * /*fileformatname*/) {
  ...
  int32 snames_table = sizeof(pef_t) + sizeof(pef_section_t)*pef.sectionCount;

...
inline uint64 extract_int64(const uchar **ptr, const uchar *end)
{
  return unpack_dq(ptr, end) - 1;
}

void append_int64(qstring &s, uint64 x);
inline void append_ea(qstring &s, ea_t x) { append_int64(s, x); }
inline ea_t extract_ea(const uchar **ptr, const uchar *end) { return extract_int64(ptr, end); }
...
void extract_regvals(
        ...
        int n,
        const uchar *regmap);
void append_regvals(qstring &s, const regval_t *values, int n, const uchar *regmap);

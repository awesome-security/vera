...
int debmod_t::set_debug_names()
{
  ...
  if ( ni == NULL || ni->addrs.empty() )
    return 1;
  int code = send_debug_names_to_ida(&ni->addrs[0], &ni->names[0], (int)ni->addrs.size());
  clear_debug_names();
  return code;
}
  ...
  int options)
{
  ...
  do
  {
    ...
    if ( dbg_read_registers(tid, -1, &ctx.saved_regs[0]) != 1 )
    {
      err = E_READREGS;
      break;
    }
        ...
        else
        {
          ...
          { // timeout out waiting for the appcall to finish
            ...
            if ( dbg_prepare_to_pause_process() <= 0 )
              break; // could not event prepare to pause, nothing we can do :(
          }
        }
      }
    ...
    if ( retregs != NULL && !retregs->empty() )
    {
      regvals_t retr;
      retr.resize(nregs);
      if ( dbg_read_registers(tid, -1, &retr[0]) <= 0 )
      {
        err = E_READREGS;
        break;
      }

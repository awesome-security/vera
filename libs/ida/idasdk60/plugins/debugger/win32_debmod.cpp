...
//--------------------------------------------------------------------------
// 1-ok, 0-failed
int idaapi win32_debmod_t::dbg_del_bpt(ea_t ea, const uchar *orig_bytes, int len)
{
  if ( orig_bytes != NULL )
  {
...
//--------------------------------------------------------------------------
int idaapi win32_debmod_t::dbg_start_process(const char *path,
                                                       const char *args,
                                                       const char *startdir,
                                                       int flags,
...
int idaapi win32_debmod_t::dbg_thread_set_step(thid_t tid)
{
  ...
  {
    QASSERT((ctx.ContextFlags & CONTEXT_CONTROL) != 0);
...
//--------------------------------------------------------------------------
// does the given module correspond to the input file?
int win32_debmod_t::is_input_module(debmod_t *sess, MODULEENTRY32 *me32, void *)
{
  win32_debmod_t *_this = (win32_debmod_t *)sess;
  char modname[MAXSTR];
  wcstr(modname, me32->szModule, sizeof(modname));
  return stricmp(modname, qbasename(_this->input_file_path.c_str())) == 0;
}
...
int win32_debmod_t::process_get_info_cb(debmod_t *sess, PROCESSENTRY32 *pe32, void *ud)
{
  ...
  if ( !accept )
  {
//    _this->dmsg("process: %x name: %S\n", pe32->th32ProcessID, pe32->szExeFile);
    accept = _this->for_each_module(pe32->th32ProcessID, is_input_module, NULL);
#ifdef UNDER_CE
    // under wince the process image name is not included in the modules list
    if ( !accept )
    {
      char modname[MAXSTR];
      wcstr(modname, pe32->szExeFile, sizeof(modname));
      accept = stricmp(modname, qbasename(_this->input_file_path.c_str())) == 0;
//    _this->dmsg("last try, inputfile_name: %s ; modname=%s\n", qbasename(_this->input_file_path), modname);
    }
#endif
  }
...
{
  if ( n == 0 ) // if the debugger updates its processes list information
  {
    input_file_path = input;
    processes.clear();

    // try to find process/module matching input_file_path
    for_each_process(process_get_info_cb, (void *)false);

    // no process was found? show everything
    if ( processes.empty() )

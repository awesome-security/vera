...
class linux_debmod_t: public BASE_DEBUGGER_MODULE
{
  ...
  {
    if ( _read_memory(-1, addr, bp.saved, sizeof(bp.saved)) == sizeof(bp.saved) )
    {
      if ( dbg_add_bpt(BPT_SOFT, addr, -1) )
      {
        bp.bpt_addr = addr;
        return true;
      }
  ...
  void new_thread(thrinfo_t *info);
  void dead_thread(int tid);

  // list of debug names not yet sent to IDA
  name_info_t pending_names;

  // wait() flags that can be used on the current system
  int wait_flags;
  pid_t qwait(int pid, int *status, bool hang);
...
public:
  ...
  int get_debug_event(const char *from, debug_event_t *event, int timeout_ms, bool autoresume=true);

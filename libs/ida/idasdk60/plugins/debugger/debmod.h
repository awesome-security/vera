...
protected:
  ...
  struct call_context_t
  {
    ...
    ea_t ctrl_ea;
    bool regs_spoiled;
    call_context_t() : regs_spoiled(false), sp(BADADDR), ctrl_ea(BADADDR) {}
  };
...
public:
  ...
  {
    return ::send_debug_names_to_ida(rpc, ea, names, qty);
  }
  int send_debug_event_to_ida(const debug_event_t *ev, int rqflags)
  {
    return ::send_debug_event_to_ida(rpc, ev, rqflags);
  }
  // -----------------------------------
  // Utility methods
  // -----------------------------------

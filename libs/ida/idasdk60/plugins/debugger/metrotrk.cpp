...
static int for_all_serial_ports(serial_port_visitor_t &v)
{
  ...
  *(FARPROC*)&lpfnSETUPDIENUMDEVICEINFO = GetProcAddress(hSetupAPI, "SetupDiEnumDeviceInfo");
  ...
  if ( dwGuids > 0 )
  {
    ...
    if ( lpfnSETUPDICLASSGUIDSFROMNAME("Ports", &guids[0], dwGuids, &dwGuids) )
    {
      // Now create a "device information set" which is required to enumerate all the ports
      hDevInfoSet = lpfnSETUPDIGETCLASSDEVS(&guids[0], NULL, NULL, DIGCF_PRESENT);
    }
  }
...
bool metrotrk_t::send_packet(void)
{
  ...
  if ( debug_debugger )
  {
    ...
    uchar nseq = pkt[2+4];
    if ( pkt.size() < 0x100 )
      show_hex(&pkt[0], pkt.size(), "\nSEND %02X %s:\n", nseq, name);
    else
      msg("\nSEND %02X %s: (%d bytes hidden)\n", nseq, name, pkt.size());
  }
  DWORD nbytes = 0;
  WriteFile(hp, &pkt[0], (DWORD)pkt.size(), &nbytes, NULL);
  if ( nbytes != pkt.size() )
  {
...
bool metrotrk_t::recv_packet(uchar *seq, int timeout)
{
  ...
  if ( debug_debugger )
  {
    ...
    show_hex(&pkt[0], s, "RECEIVED PACKET %d BYTES SEQ %x:\n", pkt.size(), *seq);
  }
  return true;
}
...
bool metrotrk_t::install_file(const char *fname, char drive)
{
  ...
  append_byte((char)toupper(drive));
  append_data(fname);
  if ( !process_packet(1) )
    return false;
...
bool metrotrk_t::install_sis_file(const char *local_sis_fname, char drive)
{
  ...
  qvector<uchar> body;
  body.resize(flen);
  qfread(fp, &body[0], flen);
  qfclose(fp);

  static const char remote_sis_fname[] = "c:\\temporary.sis";
  int mode = TrkFileOpenCreate|TrkFileOpenRead|TrkFileOpenBinary;
  int h = open_file(remote_sis_fname, mode);   QASSERT(h > 0);
  ssize_t written = write_file(h, &body[0], body.size());
  QASSERT(written == body.size()); qnotused(written);
  if ( !close_file(h, 0) )
    INTERR();
        ...
        trk_process_info_t *pi)
{
  ...
  append_data(&blob[0], blob.length());
...
bool metrotrk_t::poll_for_event(int timeout)
{
  ...
  {
    if ( !handle_notification(seq, ud) )
      INTERR(); // only notifications are allowed
  }
  return ok;
}

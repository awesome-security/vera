...
public:
  ...
  int bpt(uchar code, bpttype_t type, ea_t ea, int len);

  int send_debug_names_to_ida(const ea_t *ea, const char *const *names, int qty)
  {
    return rpc_engine_t::send_debug_names_to_ida(ea, names, qty);
  }
  ...
  {
    return rpc_engine_t::send_debug_event_to_ida(ev, rqflags);
  }

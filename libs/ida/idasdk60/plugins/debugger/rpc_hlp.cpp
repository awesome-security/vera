...
//--------------------------------------------------------------------------
static void append_regval(qstring &s, const regval_t *value)
{
  append_long(s, value->rvtype+2);
  if ( value->rvtype == RVT_INT )
  {
    append_int64(s, value->ival);
  }
...
static void extract_regval(const uchar **ptr, const uchar *end, regval_t *value)
{
  ...
  if ( value->rvtype == RVT_INT )
  {
    value->ival = extract_int64(ptr, end);
  }
  else if ( value->rvtype == RVT_FLOAT )
  {

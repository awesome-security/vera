...
{
  for ( size_t j=0; j < _bytes.size(); j++ )
  {
    bytes[j] = _bytes[j];
  }
  size = _bytes.size();
...
typedef void (*func_op_mask_t)(const mask_t *, int64 offset, op_t &op);
...
static bool byte_match_code_mask(const mask_t *mask, int64 byte, char n, char bytesize = 8)
{
...
#define OP_MASK_5 (~(make_longlong(0xFFFFFFFF,   0xFF) << (OP_BITS+8))) // effective mask
#define OP_MASK_6 (~(make_longlong(0xFFFFFFFF, 0xFFFF) << (OP_BITS+16))) // effective mask
 #define OP_OP_NULL 0
 #define OP_OP_IMM  1
...
static int64 get_masked_operand_5( int64 code, int64 mask)
{
  ...
  switch ( Switch )
  {
    ...
    case OP_OP_NOT:
      ...
      break;
    default:
      error("%a: interr: ana: get_masked_operand_5()", cmd.ea);
      break;
  }
  code &= (mask & OP_MASK_5);
  while ( (mask & 1) == 0 )
...
static int64 get_masked_operand_6( int64 code, int64 mask,bool /*bTest*/)
{
  ...
  switch ( Switch )
  {
    ...
    case OP_OP_NOT:
      ...
      break;
    default:
      error("%a: interr: ana: get_masked_operand_6()", cmd.ea);
      break;
  }
  code &= (mask & OP_MASK_6);
  while ( (mask & 1) == 0 )
...
static bool process_masks_operand(const mask_t *mask, int64 code, int64 op_mask_n, int64 &op_n, bool bTest)
{
  // initialize an operand to work on
  op_t work;
  if ( op_n < UA_MAXOP) work = cmd.Operands[int(op_n )];
  ...
  {
    if ( mask->size==5 )
      op_mask->func(mask, get_masked_operand_5(code, op_mask->mask), work); // execute the operand function
    if ( mask->size==6 )
      op_mask->func(mask, get_masked_operand_6(code, op_mask->mask,bTest), work); // execute the operand function
    bDo=true;
  }
  if ( !bDo )
  ...
  if ( work.type != o_void ) // if the operand was modified
  {
    if ( work.n == op_n ) // the function worked on the current operand
    {
      ...
      cmd.Operands[int(op_n)] = work; // save the new operand
...
static void process_masks(const mask_t *masks, ushort itype_null, bytes_c &bytes, char bytesize = 8)
{
  ...
  while ( p->mask != 0 )
  {
    bytes.reset();
    ushort nbytes = 0;
    while ( nbytes < p->size && byte_match_code_mask(p, bytes.get_next(), (uchar)nbytes, bytesize) )
      nbytes++;
    if ( nbytes == p->size ) // if bytes matches all bytes from code and mask
    {
      ...
      {
        if ( !process_masks_operand(p, code, i, op_n, (p->size==6)&&(i==2)) )
          break;
      }
      return;
    }
...
static bool find_cmd_suffix(const char *chars, ushort itype_last, bool parallel, bool name1_chars, bool name2_chars)
{
 ...
 // loop until current instruction has same params
  uint32 cmd_feature = Instructions[cmd.itype].feature;
...
#define cmd_chars(NAME,CHARS,PARALLEL,NAME1_CHARS,NAME2_CHARS)                    \
  static void NAME(const mask_t *, int64 offset, op_t &)                            \
  {                                                                               \
    if ( (offset & 1 )                                                              \
...
// set user parellel
static void cmd_UP(const mask_t *, int64, op_t &)
{
  cmd.SpecialModes |= TMS_MODE_USER_PARALLEL;
}

// built-in parallelism
static void blt_prll(const mask_t *, int64, op_t &) // count the number of actual operands
{
  cmd.Parallel = 0;
...
#define op_imm(NAME,DTYP)                                     \
  static void op_##NAME(const mask_t *, int64 offset, op_t &op) \
  {                                                           \
    op.type  = o_imm;                                         \
...
static void op_min_k4(const mask_t *, int64 offset, op_t &op)
{
  op.type  = o_imm;
  op.value = (uval_t)-offset;
  op.dtyp  = dt_byte;
...
static void op_K8(const mask_t *, int64 offset, op_t &op)
{
  op.type  = o_imm;
...
static void op_K16(const mask_t *, int64 offset, op_t &op)
{
  op.type  = o_imm;
...
/*
static void op_K23(const mask_t *, int64 offset, op_t &op)
{
  op.type  = o_imm;
...
static void op_1(const mask_t *, int64, op_t &op)
{
  op.type  = o_imm;
  op.value = 1;
  op.dtyp  = dt_byte;
...
static void op_min_1(const mask_t *, int64, op_t &op)
{
  op.type  = o_imm;
  op.value = uval_t(-1);
  op.dtyp  = dt_byte;
...
static void op_src(const mask_t *, int64 offset, op_t &op) // FSSS, FDDD
{
  op.type = o_reg;
  op.dtyp = dt_word;
...
// optional operand
static void opt_src(const mask_t *mask, int64 offset, op_t &op) // FSSS, FDDD
{
...
static void op_xsrc(const mask_t *, int64 offset, op_t &op) // XSSS, XDDD
...
static void op_Xmem(const mask_t *mask, int64 offset, op_t &op) // XXXMMM, YYYMMM
{
...
static void op_Cmem(const mask_t *mask, int64 offset, op_t &op) // mm
{
  op_CDP(mask, 0, op);
  op.tms_modifier = TMS_MODIFIER_REG + (offset & 0x3);
}

static void op_mem(const mask_t *mask, int64 offset, op_t &op, char dtyp, bool ARn_mod = false) // AAAAAAAI
{
  cmd.OpMem = 1 + op.n;
  if ( !(offset & 1) ) // @dma
      ...
      else // xxx1 xxx1
      {
        ...
        if ( ARn_mod || !arms )
        {
          switch ( bits )
          {
            ...
            case 6: // *(ARn+T0B)
            case 7: // *(ARn-T0B)
              op.tms_modifier= TMS_MODIFIER_REG_T1 + uchar(bits)-3;
              break;
          }
        }
        else
...
static void op_Smem(const mask_t *mask, int64 offset, op_t &op) // AAAAAAAI
{
  op_mem(mask, offset, op, dt_word);
}

static void op_Lmem(const mask_t *mask, int64 offset, op_t &op) // AAAAAAAI
{
  op_mem(mask, offset, op, dt_dword);
}

static void op_ARn_mod(const mask_t *mask, int64 offset, op_t &op) // AAAAAAAI
{
  op_mem(mask, offset, op, dt_word, true);
}

// @dma = bit number
static void op_Baddr(const mask_t *mask, int64 offset, op_t &op) // AAAAAAAI
{
  if ( !(offset & 1) ) // @dma
  {
    op.type  = o_imm;
...
#define fn_operator(NAME,OPERATOR)                            \
  static void fn_##NAME(const mask_t *, int64 offset, op_t &op) \
  {                                                           \
    if ( offset & 1 )                                           \
    {                                                         \
...
static void op_BitIn(const mask_t *, int64 offset, op_t &op)
{
  op.type = o_reg;
  op.dtyp = dt_byte;
...
static void op_swap(const mask_t *mask, int64 offset, op_t &op)
{
  cmd.Op1.type = o_reg;
...
static void op_l4(const mask_t *, int64 offset, op_t &op) // llll
{
  op.type = o_near;
...
static void op_L7(const mask_t *, int64 offset, op_t &op) // LLLLLLL
{
  op.type = o_near;
  op.addr = (cmd.ip + cmd.size + get_signed64(offset, 0x7F)) & 0xFFFFFF;
  op.dtyp = dt_code;
}

static void op_L8(const mask_t *, int64 offset, op_t &op) // LLLLLLLL
{
  op.type = o_near;
  op.addr = (cmd.ip + cmd.size + get_signed64(offset, 0xFF)) & 0xFFFFFF;
  op.dtyp = dt_code;
}

static void op_L16(const mask_t *, int64 offset, op_t &op) // LLLLLLLL LLLLLLLL
{
  op.type = o_near;
  op.addr = (cmd.ip + cmd.size + get_signed64(offset, 0xFFFF)) & 0xFFFFFF;
  op.dtyp = dt_code;
}

static void op_pmad(const mask_t *, int64 offset, op_t &op)
{
  op.type = o_near;
  op.addr = (cmd.ip + cmd.size + offset) & 0xFFFFFF;
  op.dtyp = dt_code;
...
static void op_P24(const mask_t *, int64, op_t &op) // get_byte included
{
  op.type = o_near;
  op.addr = (get_byte(cmd.ea+cmd.size) << 16) | (get_byte(cmd.ea+cmd.size+1) << 8) | get_byte(cmd.ea+cmd.size+2);
  op.dtyp = dt_code;
...
static void op_D16(const mask_t *, int64 offset, op_t &op)
{
  op.type = o_mem;
  op.addr = ea_t(offset);
  op.dtyp = dt_word;
...
static void op_SHIFTW(const mask_t *, int64 offset, op_t &op)
{
  op.type  = o_shift;
...
// shift left
static void shl_SHIFTW(const mask_t *, int64 offset, op_t &op)
{
  process_mask_precedent_op(op);
  op.tms_shift       = TMS_OP_SHIFTL_IMM;
  op.tms_shift_value = (signed short)get_signed64(offset, 0x3F);
  if ( ((signed short)op.tms_shift_value)<0 )
  {
    op.tms_shift       =TMS_OP_SHIFTR_IMM;
    op.tms_shift_value=-op.tms_shift_value;
  }
}

// shift left out of brackets
static void slo_SHIFTW(const mask_t *mask, int64 offset, op_t &op)
{
...
// shift right
static void shl_SHFT(const mask_t *, int64 offset, op_t &op)
{
  process_mask_precedent_op(op);
  op.tms_shift       = TMS_OP_SHIFTL_IMM;
...
static void shl_Tx(const mask_t *, int64 offset, op_t &op)
{
  process_mask_precedent_op(op);
...
static void shl_T2(const mask_t *, int64, op_t &op)
{
  process_mask_precedent_op(op);
  op.tms_shift       = TMS_OP_SHIFTL_REG;
...
static void shl_16(const mask_t *, int64, op_t &op)
{
  process_mask_precedent_op(op);
  op.tms_shift       = TMS_OP_SHIFTL_IMM;
...
static void shr(const mask_t *, int64 imm, op_t &op)
{
  process_mask_precedent_op(op);
  op.tms_shift       = TMS_OP_SHIFTR_IMM;
...
static void eq_K16(const mask_t *, int64 offset, op_t &op)
{
  process_mask_precedent_op(op);
...
static void neq_0(const mask_t *, int64, op_t &op)
{
  process_mask_precedent_op(op);
...
static void RELOP(const mask_t *, int64 offset, op_t &op) // cc
{
  process_mask_precedent_op(op);
  op.type      = o_relop;
...
static void RELOP_dst(const mask_t *mask, int64 offset, op_t &op)
{
  process_mask_precedent_op(op);
...
static void RELOP_K8(const mask_t *, int64 offset, op_t &op)
{
  process_mask_precedent_op(op);
...
static void op_cond(const mask_t *mask, int64 offset, op_t &op) // CCC CCCC
{
  op.reg = 0;
...
uchar unpack_opsdual(uchar packed)
{
  uchar lowpart[8] = { 0, 1, 2, 3, 9, 4, 10, 5};
  return ((lowpart[packed&7] | ((packed<<1) & 0x70)) << 1) | 1;
}

// unpack an "sdual" instruction
// these are probably instructions from C55x+ processor
bool unpack_sdual(ea_t ea, bytevec_t *insn1, bytevec_t *insn2)
{
    ...
    case TMS320C55:
      {
        ...
        int sdual_len = helper.altval(cmd.ea, TAG_SDUAL);
        if ( sdual_len != 0 )
        {
          // this is a second part of sdual instruction
          bytevec_t insn_bytes;
          if ( !unpack_sdual(cmd.ea - sdual_len, NULL, &insn_bytes) )
            return 0;
          bytes.set_cache(insn_bytes);
          process_masks(masks, TMS320C55_null, bytes, 8); // analyze opcode
          if ( cmd.itype == TMS320C55_null || cmd.size != insn_bytes.size() )
            return 0;
          ana_status_bits();
          cmd.SpecialModes |= TMS_MODE_USER_PARALLEL;
          return cmd.size;
        ...
        else
        {
          ...
          process_masks(masks, TMS320C55_null, bytes, 8); // analyze opcode
          if ( cmd.size != 0 && cmd.itype != TMS320C55_null )
          {
            helper.altdel(cmd.ea + cmd.size, TAG_SDUAL);
          }
          else if ( (firstbyte & 0xF8) == 0x88 )
          {
            // "sdual" instruction; unpack it
            bytevec_t insn_bytes;
            if ( !unpack_sdual(cmd.ea, &insn_bytes, NULL) )
              return 0;
            bytes.set_cache(insn_bytes);
            process_masks(masks, TMS320C55_null, bytes, 8); // analyze opcode
            if ( cmd.itype != TMS320C55_null && cmd.size == insn_bytes.size() )
            {
              ...
              helper.altset(cmd.ea + cmd.size, cmd.size, TAG_SDUAL);
              return cmd.size;
            }
          }

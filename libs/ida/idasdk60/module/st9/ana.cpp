...
static uint16 get_working_register(const int code) {
...
static const uint16 get_alu_insn(const int code) {
...
static void set_reg(op_t &op, const uint16 reg, const int flag = 0) {
    op.type = o_reg;
    op.reg = reg;
    op.dtyp = dt_byte;
    if ( flag )
...
static void set_displ(op_t &op, optype_t a1_t, uint16 a1, optype_t a2_t, uint16 a2) {
    op.type = o_displ;
    op.reg = 0;
    switch ( a1_t ) {
        case o_reg:
...
static void set_phrase(op_t &op, st9_phrases phrase, const uint16 reg1, const int reg2 = -1) {
    op.type = o_phrase;
    op.reg = reg1;
    op.specflag2 = phrase;
    op.dtyp = dt_byte;

...
//--------------------------------------------------------------------------
#define DECLARE_THIS snapman_t *_this = (snapman_t *)obj
class snapman_t
{
  struct snapdesc_t
  {
  ...
  static void idaapi edit(void *obj, uint32 n)
  {
    ...
    update_snapshot_attributes(sd->ss->filename, &_this->root, sd->ss);
  }
  ...
  static void idaapi desc(void *obj, uint32 n, char *const *arrptr)
  {
    ...
    if ( n == 0 )
    {
      qstrncpy(arrptr[0], "Date", MAXSTR);
      qstrncpy(arrptr[1], "Description", MAXSTR);
      return;
    }
  ...
  static void idaapi done_restore(const char *err_msg, void *obj)
  {
    if ( err_msg != NULL )
  ...
  void build_tree_list(snapshot_t *n, int level = 0)
  {
    if ( n != &root )
    {
      ...
      // Compute date
      char ss_date[MAXSTR];
      strftime(ss_date, sizeof(ss_date), "%Y-%m-%d %H:%M:%S", qlocaltime64(n->id));
      sd->date = ss_date;
      // Store ss
      sd->ss = n;
    }

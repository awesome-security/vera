...
size_t get_string(ea_t ea, qstring *out)
{
  ...
  {
    size_t len = get_max_ascii_length(ea, ASCSTR_UNICODE);
    if ( len > 4 )
    {
...
callgraph_t::funcinfo_t *callgraph_t::get_info(int nid)
{
  ...
  do
  {
    ...
    qstring outbuf, strings;

    if ( get_true_name(BADADDR, it_ea->second, buf, sizeof(buf)) == NULL )
    {
      /*
    ...
    else
    {
      ...
      if ( demangle(demangled, sizeof(demangled), buf, MNG_SHORT_FORM) > 0 )
      {
        outbuf.append("\n");
...
{
  iterator it;
  return find(func_ea, &it) ? *it : NULL;
}
...
bool graph_info_t::get_new_title(ea_t func_ea, qstring *out)
{
  int i;
  qstring tmp;
  ...
  for ( i=1; i < 255; i++)
  {
    tmp.sprnt("Call graph of: %s (%d)", func_name, i);

    if ( find_tform(tmp.c_str()) == NULL)
    {
      *out = tmp; //->sprnt("Call graph %d of: %s", callgraph_num, func_name);
      break;
    }
  }
...
//--------------------------------------------------------------------------
void idaapi callgraph_t::jump_disasm(void *ud, int code, va_list va)
{
  DECLARE_GI_VARS;
  qnotused(code);
...
//--------------------------------------------------------------------------
static const char *NODE_NAME = "$ proximity browser";
...
static int idaapi options_cb(int fid, form_actions_t &fa)
{
  ...
  {
    if ( !fa.get_field_value(FIELD_ID_CHILDS, &opt) )
      INTERR(0);
  ...
  {
    if ( !fa.get_field_value(FIELD_ID_FATHERS, &opt) )
      INTERR(0);
    ...
    if ( opt > MAX_CALLERS_LEVEL )
    {
      ...
      fa.set_field_value(FIELD_ID_FATHERS, &opt);
    }
  }
...
static void idaapi show_hidden_desc(void *obj,uint32 n,char * const *arrptr)
{
  ...
  if ( get_true_name(BADADDR, ea, func_name, sizeof(func_name)) != NULL )
  {
    qsnprintf(arrptr[0], MAXSTR, "%s", func_name);
...
void idaapi run(int arg)
{
  ...
  if ( hwnd != NULL )
  {
    ...
    if ( gi != NULL )
    {
      ...
      gi->mark_for_refresh();
      gi->form = form;
      gi->gv = create_graph_viewer(form, id, gi->fg.gr_callback, gi, 0);
      ...
      if ( gi->gv != NULL )
      {
        ...
        viewer_add_menu_item(gi->gv, "Options", gi->fg.menu_options_cb, gi, "O", 0);
        viewer_add_menu_item(gi->gv, "Refresh", gi->fg.menu_refresh_cb, gi, "R", 0);
        viewer_add_menu_item(gi->gv, "-", donothing, gi, NULL, 0);
        viewer_add_menu_item(gi->gv, "Jump to xref", gi->fg.menu_jumpxref_cb, gi, "X", 0);
        viewer_add_menu_item(gi->gv, "Jump to address", gi->fg.menu_jumpaddr_cb, gi, "G", 0);
        viewer_add_menu_item(gi->gv, "Jump to function", gi->fg.menu_jump_cb, gi, "SPACE", 0);
        viewer_add_menu_item(gi->gv, "Jump to previous node", gi->fg.menu_back_cb, gi, "ESC", 0);
        viewer_add_menu_item(gi->gv, "Jump to next node", gi->fg.menu_forward_cb, gi, "Ctrl+Enter", 0);
        viewer_add_menu_item(gi->gv, "-", donothing, gi, NULL, 0);
        viewer_add_menu_item(gi->gv, "Center node", gi->fg.menu_center_cb, gi, "Enter", 0);
        viewer_add_menu_item(gi->gv, "Select node", gi->fg.menu_select_cb, gi, "Ctrl+L", 0);
        viewer_add_menu_item(gi->gv, "Goto to first node", gi->fg.menu_home_cb, gi, "H", 0);
        viewer_add_menu_item(gi->gv, "Search first", gi->fg.menu_searchfirst_cb, gi, "S", 0);
        viewer_add_menu_item(gi->gv, "Search next", gi->fg.menu_searchnext_cb, gi, "N", 0);
        viewer_add_menu_item(gi->gv, "Hide selected node", gi->fg.menu_hidenode_cb, gi, NULL, 0);
        viewer_add_menu_item(gi->gv, "Show hidden node", gi->fg.menu_showhidden_cb, gi, NULL, 0);
        viewer_add_menu_item(gi->gv, "Show all nodes", gi->fg.menu_showall_cb, gi, NULL, 0);
      }
      else
      {
        graph_info_t::destroy(gi);
...
int idaapi init(void)
{
  ...
  if ( !add_menu_item(CMD_MENU "Function calls", CMD_NAME, "Ctrl+Shift+B", SETMENU_APP, run_plugin, NULL) )
  {

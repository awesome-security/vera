...
//---------------------------------------------------------------------------
// A basic function to parse an application path that may include
  ...
  bool *append_out)
{
  ...
  char *_path = qstrdup(path);
  if ( _path == NULL )
    return false;

  size_t npart = 0;
  for ( char *p=strtok(_path, "<>");
    p != NULL && npart < 3;
    p = strtok(NULL, delim), ++npart )
  {
    // Take current token index
    ptrdiff_t idx = p - _path - 1;

    // Skip leading spaces and trailing spaces
    p = skipSpaces(p);
    trim(p);
    ...
    else
    {
      ...
      // Was it an output redirection?
      if ( path[idx] == '>' )
      {
        // ">>" was used?
        *append_out = idx > 1 && path[idx-1] == '>';
        *redir_out = p;
      }
      // Was it an input redirection?
      else if ( path[idx] == '<' )
        *redir_in = p;
    }
  }

...
bool win32_debmod_t::create_process(
    ...
    bool append_out)
{
  // If redirection is passed, then the two parameters should be passed
  // Why? Because, say output redirection is not enabled, what to pass to si.hStdOutput?
  // We could use GetStdHandle(STD_OUTPUT_HANDLE) but it will return NULL because idaq does
  // not have a console window
  if ( (redir_in == NULL) != (redir_out == NULL) )
  {
    dwarning("Please use both input and output redirection!");
  ...
  // Make I/O redirection file handles inheritable
  SECURITY_ATTRIBUTES redir_sa = { sizeof(SECURITY_ATTRIBUTES), NULL, TRUE };

  // Open input redirection
  if ( redir_in != NULL )
  {
    redirin_handle = CreateFile(
      redir_in,
      GENERIC_READ,
      FILE_SHARE_READ,
      &redir_sa,
      ...
      0,
      NULL);
    if ( redirin_handle == NULL )
    {
      dwarning("Could not open input redirection file: %s", redir_in);
      return false;
    }
    si.hStdInput = redirin_handle;
  }

  // Create/Open output redirection
  if ( redir_out != NULL )
  {
    redirout_handle = CreateFile(
      redir_out,
      append_out ? FILE_APPEND_DATA : GENERIC_WRITE,
      FILE_SHARE_READ,
      &redir_sa,
      OPEN_ALWAYS,
      0,
    ...
    if ( redirout_handle == NULL )
    {
      // Close input redirection file
      myCloseHandle(redirin_handle);

      dwarning("Could not %s output redirection file: %s", append_out ? "open" : "create", redir_out);
      return false;
    }
    si.hStdError = si.hStdOutput = redirout_handle;
  }

  // Redirection used?
  if ( redir_in != NULL || redir_out != NULL )
  {
    // Use the passed handles
  ...
  return CreateProcess(
    ...
    si.dwFlags == 0 ? FALSE : TRUE,      // handle inheritance flag

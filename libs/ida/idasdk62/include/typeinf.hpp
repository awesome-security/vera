...
//--------------------------------------------------------------------------
// DEFINITIONS FOR THE KERNEL (NON PUBLIC)
//--------------------------------------------------------------------------
bool append_type_name(
        const til_t *ti,
        qtype *type,
        qtype *fields,
        const char *name,
        const type_t *subtype,
        const p_list *subfields);

//--------------------------------------------------------------------------
int for_all_types_ex(const til_t *ti, const type_t **ptype, type_visitor_t &tv);

typedef bool (*input_exe_reader_t)(uint64 fileOffset, uint32 count, void *buffer, uint32 *read);
typedef bool (*input_mem_reader_t)(ea_t ea, uint32 count, void *buffer, uint32 *read);

//--------------------------------------------------------------------------
// input_path: EXE or PDB path
// dpath: PDB download path (MS servers are used)
// user_spath: full search path (can have srv* and so on)
// flags: see HTI_... above (not used for now)
// load_address: when loading file from memory, base address to be used for passing to mem_reader
int pdb2ti(til_t *ti,

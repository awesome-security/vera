...
#define  VT_REF        10       // Reference
  ...
  void _set_string(const char *str, size_t len)
  {
    vtype = VT_STR2;
    new (&qstr()) qstring(str, len);
  }
  ...
  void set_string(const char *str, size_t len) { clear(); _set_string(str, len); }
  void set_string(const char *str) { clear(); _set_string(str); }
...
struct extlang_t                // External language
{
  ...
  bool (idaapi *run_statements)(// Compile and execute a string with statements
...
// does the run_string() extlang callback exist?
...
idaman bool ida_export run_statements(
        ...
        char *errbuf,                   // out: error message if evaluation fails
        size_t errbufsize,              // in: size of the error buffer
        const extlang_t *elang = NULL); // in: optional extlang to use.
...
bool idaapi run_statements_idc(const char *str, char *errbuf, size_t errbufsize);
void idaapi init_idc(void);
void idaapi term_idc(void);
void idaapi create_default_idc_classes(void);

extfun_t *find_builtin_idc_func(const char *name);
void insn_to_idc(const class insn_t &ins, idc_value_t &r);

...
idaman asize_t ida_export get_data_type_size(flags_t F, const opinfo_t *ti);
#define get_data_type_size_by_flags get_data_elsize
inline bool idaapi f_isUnknown(flags_t F, void *) { return isUnknown(F); }
idaman const uint32 ida_export_data power2[32];    // Powers of 2, from 2^0 to 2^31
idaman const uint32 ida_export_data lowbits[33];   // Low-order bits, from 0 to 32
...
idaman opinfo_t *ida_export get_typeinfo(ea_t ea, int n, flags_t flags, opinfo_t *buf);
idaman bool ida_export set_typeinfo(ea_t ea, int n, flags_t flag, const opinfo_t *ti);
// same as get_ascii_contents2, but always convert Unicode to ASCII
idaman bool ida_export get_ascii_contents(ea_t ea, size_t len, int32 type, char *buf, size_t bufsize);
#endif

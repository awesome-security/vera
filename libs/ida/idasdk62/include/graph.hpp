...
#define COLLAPSED_NODE 0x80000000
  ...
  bool idaapi is_dot_node(int node) const { return (node_flags[node] & MTG_DOT_NODE) != 0; }
  bool idaapi is_group_node(int node) const { return (node_flags[node] & MTG_GROUP_NODE) != 0; }
...
// Add graph menu item
//          gv       - graph viewer object
//          title    - menu title (pass '-' to create a separator).
//          callback - the menu item callback
//          ud       - user data that will be passed back to the callback
//          hotkey   - hotkey associated with the menu item
//          flags    - not used

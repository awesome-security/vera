...
local void swap_version_min_command(struct version_min_command *ec)
{
  ...
  ec->reserved = swap32(ec->reserved);
}
...
//--------------------------------------------------------------------------
bool macho_file_t::set_subfile(uint n, size_t filesize)
{
...
bool macho_file_t::parse_load_commands()
{
  ...
  load_commands.clear();
  uint32 i;
  for ( i = 0 ; i < mh.ncmds && begin < commands_end; i++ )
  {
...
void macho_file_t::visit_function_starts(function_starts_visitor_t &v)
{
  ...
  if ( visit_load_commands(vfs) && fs.datasize != 0 )
  {
    ...
    while ( p < end )
    {
      uint64_t delta = unpack_uleb128(&p, end);
      address += delta;
...
bool dyld_cache_t::parse_header()
{
  ...
  if ( qlread(li, &mappings.at(0), sizeof(dyld_cache_mapping_info) * header.mappingCount) != sizeof(dyld_cache_mapping_info) * header.mappingCount )
    return false;
  if ( qlseek(li, header.imagesOffset) != header.imagesOffset )
    return false;
  image_infos.resize(header.imagesCount);
  if ( qlread(li, &image_infos.at(0), sizeof(dyld_cache_image_info) * header.imagesCount) != sizeof(dyld_cache_image_info) * header.imagesCount )
    return false;

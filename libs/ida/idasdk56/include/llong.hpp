...
idaman char *ida_export print(uint32 l,uint32 h,char *buf,int bufsize,int radix,int issigned);
...
//#define low(x)   uint32(x)
//#define high(x)  uint32(x>>32)
#endif
...
#if defined(__WATCOMC__) || defined(__VC__)
#define WC4(x)  low(x)
#else
#define WC4(x)  (x)
#endif

...
typedef AS_PRINTF(1, 2) int idaapi printer_t(const char *format, ...);
...
idaman bool ida_export deref_ptr(til_t *ti, const type_t *type, ea_t *ptr_ea, ea_t *closure_obj=NULL);
...
void sync_from_til(til_t *ti, const char *name, const type_t *type);
...
idaman error_t ida_export pack_object_to_bv(
        ...
        void *objoff,         // NULL - append object to 'bytes'
                              ...
                              //   out: data for object, *(int32*)objoff == -1
        int pio_flags=0);


// get structure member at the specified offset
idaman bool ida_export get_strmem(
        const til_t *til,
        ...
        qstring *name,        // out: member name
        qtype *ftype=NULL,    // out: member type
        qtype *fnames=NULL,   // out: member field names
        qstring *sname=NULL); // out: structure type name

// get structure member with the specified name
idaman bool ida_export get_strmem_by_name(
        const til_t *til,
        const type_t *type,   // in: type
        const p_list *fields, // in: fields. for typedefs may be NULL
        const char *name,     // in: member name
        asize_t *offset,      // out: member offset
        qtype *ftype=NULL,    // out: member type
        qtype *fnames=NULL,   // out: member field names
        qstring *sname=NULL); // out: structure type name

...
//      to populate XML variables, select the default processor, etc.
//      It is called when "File, New" command is selected and allows the loader
//      to initialize itself before wizard pages are displayed.
...
void add_fileregion(ea_t ea1,ea_t ea2,int32 fpos);      // called by the kernel
...
class place_t;
int32 print_all_places(FILE *fp,place_t *pl, html_line_cb_t *line_cb = NULL);
...
idaman bool ida_export save_database(const char *outfile, bool delete_unpacked);

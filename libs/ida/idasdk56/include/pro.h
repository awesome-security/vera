...
#define IDA_SDK_VERSION      560        // IDA SDK v5.6
...
idaman NORETURN AS_PRINTF(3, 0) void ida_export vinterr(const char *file, int line, const char *format, va_list va);
...
#else
#define va_assign(dst, src) dst = (va_list)src
#endif
...
idaman AS_PRINTF(3, 0) int ida_export qvsnprintf(char *buffer, size_t n, const char *format, va_list va);
idaman AS_SCANF (2, 0) int ida_export qvsscanf(const char *input, const char *format, va_list va);
idaman AS_PRINTF(3, 4) int ida_export qsnprintf(char *buffer, size_t n, const char *format, ...);
idaman AS_PRINTF(3, 4) int ida_export append_snprintf(char *buf, char *end, const char *format, ...);
idaman AS_SCANF (2, 3) int ida_export qsscanf(const char *input, const char *format, ...);
...
idaman void ida_export swap_value(void *dst, const void *src, int size);
idaman void ida_export reloc_value(void *value, int size, adiff_t delta, bool mf);
...
// We redefine new/delete to use common memory pool for all plugins
#if defined(__BORLANDC__)
#define MEMORYAPI _RTLENTRY _EXPFUNC32
#elif defined(_MSC_VER)
#define MEMORYAPI __CRTDECL
#else
#define MEMORYAPI
#endif
inline void *MEMORYAPI operator new(size_t size)
{
  return qalloc_or_throw(size ? size : 1);
}

inline void MEMORYAPI operator delete(void *blk)
{
  qfree(blk);
}
...
public:
...
#else // GNU compiler creates some STL objects that are non-agnostic to their positions
      // We have to explicitly move them by constructing and destructing. It is a pity.
      T *new_array = (T*)qalloc_or_throw(b);
      move(new_array, array, n);
      qfree(array);
      array = new_array;
#endif
      alloc = m;
  ...
  void move(T *dst, T *src, size_t cnt)
  {
...
template <> inline void qvector<int>::move(int *dst, int *src, size_t cnt) { memcpy(dst, src, cnt*sizeof(int)); }
template <> inline void qvector<uint>::move(uint *dst, uint *src, size_t cnt) { memcpy(dst, src, cnt*sizeof(int)); }
template <> inline void qvector<char>::move(char *dst, char *src, size_t cnt) { memcpy(dst, src, cnt); }
template <> inline void qvector<uchar>::move(uchar *dst, uchar *src, size_t cnt) { memcpy(dst, src, cnt); }

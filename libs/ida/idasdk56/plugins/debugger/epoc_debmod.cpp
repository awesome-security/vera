...
{
  qnotused(type);
  QASSERT(type == BPT_SOFT); // other types are not supported yet
...
//--------------------------------------------------------------------------
int idaapi epoc_debmod_t::dbg_del_bpt(ea_t ea, const uchar * /*orig_bytes*/, int /*len*/)
{
  bpts_t::iterator p = bpts.find(ea);
  if ( p == bpts.end() )
    return 0; // failed
...
//--------------------------------------------------------------------------
int idaapi epoc_debmod_t::dbg_get_debug_event(debug_event_t *event, bool ida_is_idle)
{
  if ( event == NULL )
    return 0;
...
//--------------------------------------------------------------------------
int idaapi epoc_debmod_t::dbg_thread_read_registers(thid_t tid, regval_t *values, int n)
{
  uint32 rvals[17];
...
//--------------------------------------------------------------------------
int idaapi epoc_debmod_t::dbg_thread_write_register(thid_t tid, int reg_idx, const regval_t *value)
{

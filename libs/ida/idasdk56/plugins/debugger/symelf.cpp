        ...
        symbol_visitor_t &sv)
{
  ...
  return sv.visit_symbol(st_value, name);
}

//--------------------------------------------------------------------------
static int load_symbols32(uint32 pos, uint32 size, int namsec, symbol_visitor_t &sv)
{
  int code = 0;
  ...
  for ( int i = 1; i < cnt && code == 0; i++ )
  {
    ...
    if(lread4bytes(li,          &st.st_name,  mf) ||
       ...
       lread2bytes(li,          &st.st_shndx, mf))     return -1;

    code = handle_symbol(st.st_shndx, st.st_info, st.st_name, st.st_value, namsec, sv);
  }
  return code;
}

static int load_symbols32(ushort symsec, int namsec, symbol_visitor_t &sv)
{
  Elf32_Shdr *sh = shdr32+symsec;
  return load_symbols32(sh->sh_offset, sh->sh_size, namsec, sv);
}

//--------------------------------------------------------------------------
static int load_symbols64(uint32 pos, uint32 size, int namsec, symbol_visitor_t &sv)
{
  int code = 0;
  ...
  for ( int i = 1; i < cnt && code == 0; i++ )
  {
    ...
    if(lread4bytes(li, &st.st_name,  mf) ||
       ...
       lread8bytes(li, &st.st_size,  mf) ) return -1;
...
static int load_symbols64(ushort symsec,ushort namsec, symbol_visitor_t &sv)
{
  Elf64_Shdr *sh = shdr64+symsec;
  return load_symbols64(low(sh->sh_offset), sh->sh_size, namsec, sv);
}
...
//--------------------------------------------------------------------------
static int _load_elf_symbols(linput_t *_li, symbol_visitor_t &sv)
{
  init_elf_vars();
...
//      e_ident[EI_DATA], ELFDATA2LSB, ELFDATA2MSB);
  ...
  if(!read_uword(&ehdr.e_entry) ||
     !read_uword(&ehdr.e_phoff) ||
     !read_uword(&ehdr.e_shoff) ||
     ...
     lread2bytes(li, &ehdr.e_shstrndx, mf) ||
              (ehdr.e_shstrndx && ehdr.e_shstrndx >= ehdr.e_shnum)) return -1;
...
//    warning("SHT size or offset is invalid");
  ...
  if ( ehdr.e_shnum && ehdr.e_shentsize )
  {
    if ( elf64 )
    {
      shdr64 = (Elf64_Shdr *)qalloc(ehdr.e_shnum * sizeof(Elf64_Shdr));
    ...
    {
      shdr32 = (Elf32_Shdr *)qalloc(ehdr.e_shnum * sizeof(Elf32_Shdr));
  ...
  if ( dyn_size )
  {
    read_dyninfo(dyn_offset, dyn_size, &dyninfo);
    parse_dyninfo(dyninfo);
  }

  if ( int_sec && (sv.velf & VISIT_INTERP) != 0 )
  {
    load_name2(int_sec, 0);
    code = sv.visit_interp(name);
    if ( code != 0 )
      goto ret;
  }
  ...
  if ( (sv.velf & VISIT_SYMBOLS) != 0 )
  {
    if ( sym_sec || dsm_sec )
    {
      ...
      if ( sym_sec )
        if ( elf64 )
          code = load_symbols64(sym_sec, str_sec, sv);
        else
          code = load_symbols32(sym_sec, str_sec, sv);

      if ( code == 0 && dsm_sec )
        if ( elf64 )
          code = load_symbols64(dsm_sec, dst_sec, sv);
        else
          code = load_symbols32(dsm_sec, dst_sec, sv);
    }
    else if ( dynsym_size )
    {
      if ( elf64 )
        code = load_symbols64(dynsym_off, dynsym_size, -1, sv);
      else
        code = load_symbols32(dynsym_off, dynsym_size, -1, sv);
    }
  }
  ...
  if ( (sv.velf & VISIT_DYNINFO) != 0 && !dyninfo.empty() )
  {
    ...
    for ( int i=0; code == 0 && i < dyninfo.size(); i++ )
    {
      ...
      switch ( dyn.d_tag )
      {
        ...
        case DT_RUNPATH:
        case DT_NEEDED:
          load_name2(link, dyn.d_un);
          break;
      }
      code = sv.visit_dyninfo(dyn.d_tag, name, dyn.d_un);
    }
  }

...
idarpc_stream_t *init_client_irs(const char *hostname, int port_number)
{
  ...
  if ( getaddrinfo(hostname, port, &ai, &res) != 0 )
  { // failed to resolve the name
    last_error_code = get_network_error();
  }
  else
  {
    for ( e = res; !ok && e != NULL; e = e->ai_next )
    {
      ...
      char uport[33];
      if ( getnameinfo(e->ai_addr, e->ai_addrlen, uaddr, INET6_ADDRSTRLEN,
                        uport, 32, NI_NUMERICHOST | NI_NUMERICSERV) != 0 )
      {
        //msg("getaddrinfo failed: %s\n", winerr(get_network_error()));
        last_error_code = get_network_error();
        continue;
      }
      //msg("Connecting to %s, port %s\n", uaddr, uport);
      sock = socket(e->ai_family, e->ai_socktype, e->ai_protocol);
      if ( sock == INVALID_SOCKET )
      {
        //msg("socket(family=%d) failed: %s\n", e->ai_family, winerr(get_network_error()));
        last_error_code = get_network_error();
        continue;
      }
      ...
      {
        last_error_code = get_network_error();
        //msg("connect(family=%d) failed: %s\n", e->ai_family, winerr(get_network_error()));
        closesocket(sock);
        continue;
      }
      //msg("connect(family=%d) OK\n", e->ai_family);
      ok = true;
    }
    freeaddrinfo(res);
  }
  ...
  {
    warning("ICON ERROR\nAUTOHIDE NONE\n"
            "Could not connect to %s: %s", hostname, winerr(last_error_code));
    return NULL;
  }

...
//
static void doRelocs(int16 baseaddr, bool dosegs, netnode ovr_info)
{
  fixup_data_t fd;
  ...
  for(ea_t xEA=ovr_info.alt1st(); xEA !=BADADDR; xEA=ovr_info.altnxt(xEA))
  {
    ...
    int16 base = ushort(curval + baseaddr);
    put_word(xEA, base);
    fd.sel  = base;
    set_fixup(xEA, &fd);
    if ( dosegs )
...
//    int num_shifted = ovr_info.altshift(current_base, new_base, MAXADDR);

...
void do_relocation_sparc(linput_t *li, aout &ctx, uint32 off, uint32 len, int seg)
{
    ...
    if ( relcount == 0 )
      return;
    reltab = (reloc_info_sparc *) qalloc(relcount * sizeof(reloc_info_sparc));
    if (reltab == NULL)
    {
...
void load_syms(linput_t *li, aout &ctx)
{
    ...
    lread4bytes(li, &strlen, ctx.msb);
#ifdef DEBUG
    msg("symoff=0x%08x symlen=0x%08x stroff=0x%08x strlen=0x%08x\n",
        ctx.symoff, ctx.ex.a_syms, ctx.stroff, strlen);
#endif

    // load string table
    strtab = (char *) qalloc(strlen+1);
    if (strtab == NULL)
    {
    ...
    qlseek(li, ctx.stroff);
    lreadbytes(li, strtab, strlen, false);
    strtab[strlen] = '\0'; // make sure a malformed file doesn't have an unterminated string

    // load symbol table
    uint32 extern_count = 0;
    ctx.symcount = ctx.ex.a_syms / sizeof(nlist);
    symtab = (nlist *) qalloc(ctx.symcount * sizeof(nlist));
    if (symtab == 0)
    {
...
void load_file(linput_t *li, ushort /*neflag*/, const char * /*fileformatname*/)
{
  ...
  case PLFM_SPARC:
    ...
    ctx.dreloff = N_TRELOFF_SPARC(ex);

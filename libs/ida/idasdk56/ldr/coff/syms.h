...
//    char *_n_nptr[2];         /* allows for overlaying */
  ...
  int16 n_scnum;                /* section number */
  uint16 n_type;                /* type and derived type */
  uchar n_sclass;               /* storage class */
  uchar n_numaux;               /* number of aux. entries */
};
#define IDA_SYM_SZ    (SYMNMLEN + 2*4 + 2 + 2*1)

struct ida_sym_t // also is form of coff_sym_t in COFF+
{
  union {
...
struct ic_seginfo_t
{
  const ida_sym_t *sym; // comdat symbol
  ea_t start;           // segment start address, BADADDR - not loaded
  adiff_t delta;        // difference between the loaded address
                        // and address in the file
  ic_seginfo_t(void) : sym(NULL), start(BADADDR), delta(0) {}
};
extern qvector<ic_seginfo_t> seginfo;

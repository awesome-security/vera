									...
									int redNumber)
{
...
#ifdef OGDF_DEBUG
		checkLeaf->status(FULL); // For debugging and visualization purpose only
#endif
	...
	*/
	...
	{
		char number[8];
		ogdf::sprintf(number, 8, "%d", redNumber);
		PQTree<T,whaInfo*,Y>::printOutCurrentTree("WHA_com",number);
	}
	#endif
...
void MaxSequencePQTree<T,Y>::clientDefinedEmptyNode(PQNode<T,whaInfo*,Y>* nodePtr)
{
	...
	{	
		...
		emptyNode(nodePtr);
	else if (nodePtr->status() == EMPTY)
		emptyNode(nodePtr);
	else
	{
		OGDF_ASSERT(false) // Node has an invalid status 
	}
}
...
void MaxSequencePQTree<T,Y>::emptyAllPertinentNodes()
{
	...
	while (!cleanUp.empty())
	{
		...
		if (nodePtr->status() == WHA_DELETE && nodePtr->type() == LEAF)
		{
			CleanNode(nodePtr);
			delete nodePtr;
		}

		else if (nodePtr->getNodeInfo() != 0)
		{
			 ...
			 PQNode<T,whaInfo*,Y> *hChild2Sib)     
{
	...
	else if (hChild2->status() == PARTIAL)
		hChild2->getNodeInfo()->userStructInfo()->m_deleteTyp = H_TYPE;
	else
	{
		OGDF_ASSERT(false) //1. node of sequence is EMPTY
	}
...

{
	...
	else
	{
		...
		while (currentNode->parent()->status() == ELIMINATED)
		{
			...
			OGDF_ASSERT(currentNode) //endmost child with no 
				                      // valid parent pointer detected
		}
		while (!L.empty())
